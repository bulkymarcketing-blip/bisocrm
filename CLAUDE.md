...
- **Package CHILD** (NEW, optional): a normal item plus `parentKey:<parent key>`. Carries the full item field set; package total = Σ children. A package is **pinned to ONE ceremony** — `docSectionFor` groups a child under its parent's `pkgCeremony`; children are positive priced lines (package/order discounts use the existing top-level discount mechanisms, not negative child lines).
