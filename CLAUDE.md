...
- **Next: Phase 7 (Data screens — Quotations / Profiles / Analytics; Invoices + Finance carry the look via Cycle A).** Reuse the Phase-4 `_brief*` vocabulary + the Phase-5/6 `.fpill` pill + flat-card patterns (see below).
...
1. **Data screens** — **Quotations, Profiles, Analytics** (Invoices + Finance already carry this look via Cycle A) — shared table/list pattern → cleaner rows/cards on the established tokens. (Confirmed is dropped — see Status.)
