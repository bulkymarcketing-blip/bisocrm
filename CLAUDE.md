...
1. **Pipeline — DONE.** Presentation + view-state only (leads still group by numeric `stage`; CTA actions unchanged). The pastel collapsible lanes were retired (`.pipeline`/`.stage-col`/`.shdr`/`.sbdy`, `SC[]`/`.p0–.p4`, `toggleLane`/`toggleLaneHelp`/`STAGE_DEFS`/`lane-help*` all removed) and replaced with a **stage-pill row** that **reuses `.fpill`/`.fpill.on`** (gold active, like the Brief): one pill per stage with its count, **default-selects the first stage**, pills **wrap** (no horizontal scroll); active stage held in **`window._pipeStage`** (session; default 0) via **`setPipeStage(i)`**, and the selected stage's leads render as a **flat stack of `lCard`s**. The Pipeline is now **stages-only** — the `renderTodaysActions` action widget was dropped from `rPipeline` (it's the Brief's job; `renderTodaysActions` itself is unchanged apart from the relabel below). Page title **"Lead Pipeline" → "Pipeline"**. **Stage rename (display-only, stage is a number):** `STAGES[2]` "Consultation Booked" → **"Visit Booked"**, `STAGES[3]` "Consultation Done" → **"Visited"**, with the card CTA "Mark consult done" → **"Mark visited"** and the Brief Quotes-to-send copy ("Consultation Xd ago" → "Visited Xd ago", meta "consultation done" → "visited"). **PENDING separate task:** the full consultation→visit wording sweep (the appointment reason `'Consultation'`, reminders, help/guide text) is intentionally NOT done yet. **Card treatment:** flat `--surf` + 1px hairline, **no red `lc-overdue` bar** (overdue shows via badges), **navy `.lcard-cta-full`** hero next-step button, quiet borderless `.lcard-icon-btn`, **inline SVG line icons (no Tabler CDN)** — emoji removed everywhere incl. the expanded view; tamed tag row (idle/age → quiet amber `.lidle` clock text not a red pill; reminder badge red only when Overdue/Today, amber Tomorrow, neutral later; Quoted/Wedding-in → neutral `.lchip`; attempt-count moved to the expanded view) + a muted **"Rs X quoted"** `.lamt` line on quoted cards. Closed-lost restyled (1px, sentence-case "Closed–lost"). **Sims:** `node sim/pipeline.sim.js` (pills/cards/widget-gone/rename + regression that `computeTodaysActions`/`_brief*`/`pipelineMetrics` are byte-identical and the Brief is unchanged except the relabel); `dailyBrief.sim.js`'s `[I]` proof is now relabel-aware.
